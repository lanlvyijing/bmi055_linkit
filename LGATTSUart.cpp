...
static LGATTServiceInfo g_uart_decl[] =
{
    ...
    {TYPE_CHARACTERISTIC, "08590F7F-DB05-467E-8757-72F6FAEB13D4", FALSE, VM_GATT_CHAR_PROP_NOTIFY |VM_GATT_CHAR_PROP_WRITE | VM_GATT_CHAR_PROP_READ, VM_GATT_PERM_WRITE, 0}, // rx for periphral; tx for central
    {TYPE_END, 0, 0, 0, 0, 0}
...
boolean LGATTSUart::onCharacteristicAdded(LGATTAttributeData &data)
{
    ...
    */
    ...
    APP_LOG("LGATTSUart::onCharacteristicAdded data failed=[%d] uuid[12] = [0x%x] len=[%d]", data.failed, uuid->uuid[12], uuid->len);
    if (!data.failed)
    {
        if (0x7E == uuid->uuid[12]) //use the uuid[12] to distinguish the Characteristic type
        {
        ...
        APP_LOG("LGATTSUart::onCharacteristicAdded uuid[12] = [0x%x]", uuid->uuid[12]);     
    }
    else
    {
      APP_LOG("LGATTSUart:: data failed");
    }
    return true;
}
...
boolean LGATTSUart::onRead(LGATTReadRequest &data)
{
    ...
    if (_connected)
    {
        ...
        APP_LOG("LGATTSUart::onRead length:[%d]  onRead value:[%s]", value.len, value.value);
